...
class SlackApiServiceProvider extends ServiceProvider
{
     ...
     * @var string
     */
    protected $contractsNamespace = 'Provectus\SlackApi\Contracts';

    /**
     * Default methods namespace.
     * @var string
     */
    protected $methodsNamespace = 'Provectus\SlackApi\Methods';
    ...
    public function register()
    {
        ...
        $this->app->singleton('Provectus\SlackApi\Contracts\SlackApi', function () {
            $api = new SlackApi(null, config('services.slack.token'));

            return $api;
        });

        $this->app->alias('Provectus\SlackApi\Contracts\SlackApi', 'slack.api');

        foreach ($this->methods as $method) {
            $this->registerSlackMethod($method);
        }

        $this->app->alias('Provectus\SlackApi\Contracts\SlackInstantMessage', 'slack.im');

        $this->app->alias('Provectus\SlackApi\Contracts\SlackRealTimeMessage', 'slack.rtm');
    }
